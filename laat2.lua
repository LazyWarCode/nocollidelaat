...
function GM:ShouldCollide( laat2, ply )
        return false;
    end

function DisableVehicleDamage( victim, attacker )
    if ( attacker:IsVehicle() or attacker:GetClass() == "prop_vehicle_jeep" ) then
        return false
    end
end
hook.Add("PlayerShouldTakeDamage","DisableVehicleDamage",DisableVehicleDamage)
...
hook.Add("PlayerLeaveVehicle", "LAATSeatExit", function(p,v)
	if(IsValid(p) and IsValid(v)) then
		if(v.IsLAATSeat) then
			...
			if(IsValid(e)) then
				if(v.LeftSide) then
					p:SetPos(e:GetPos()+e:GetRight()*65+e:GetUp()*20+e:GetForward()*-40);
